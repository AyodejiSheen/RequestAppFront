...
export const AcceptedReq = () => {
    ...
    useEffect(() => {
        if (authState){
            ...
            axios.get(`${baseUrl.baseUrl}/request/accepted-requests/${Id}`, {
                headers: {
                    accessToken: token
                }
            }).then((response) => {
    ...
    return (
        <>
            ...
            <div className="container mx-auto pb-16 pt-2">
                <h2 className="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">Your Accepted Requests</h2>
...

                            {
                                accept.length === 0 ? (
                                    <div className=" py-20 mx-auto text-center">
                                        <h2 className="mt-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">You haven't accept any request yet</h2>
                                        <p className="mb-6 mt-2.5 dark:text-gray-400">Accept one by following the link below</p>
                                    ...
                                    <div className="xl:grid xl:grid-cols-2 2xl:grid-cols-3 gap-x-16 gap-y-10 mt-8 space-y-8 xl:space-y-0">
                                        {
                                            accept.slice(0).reverse().map((each, i) => {
                                                return (
